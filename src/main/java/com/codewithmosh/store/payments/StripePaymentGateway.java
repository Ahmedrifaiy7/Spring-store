...
public class StripePaymentGateway implements PaymentGateway {
    ...
    private Long extractOrderId(Event event) {
        var stripeObject = event.getDataObjectDeserializer().getObject().orElseThrow(
                () -> new PaymentException("could not deserialize stripe event. check thestripe api version."));
    ...
    private SessionCreateParams.LineItem.PriceData createPriceData(OrderItem item) {
        return SessionCreateParams.LineItem.PriceData.builder()
                ...
                .setProductData(createProductDate(item))
                .build();
    }

    private SessionCreateParams.LineItem.PriceData.ProductData createProductDate(OrderItem item) {
